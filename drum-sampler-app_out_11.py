...
class DrumSamplerApp(Gtk.Window):
    ...
    def on_button_toggled(self, button, instrument, step):
        if self.advanced_sequencer_mode:
            ...
            if is_active and 'note_count' not in step_data:
                step_data['note_count'] = 1  # Domyślna wartość dla nowych kroków
        ...
        for inst in self.instruments:
            if self.advanced_sequencer_mode:
                if len(self.patterns[inst]) < pattern_length:
                    self.patterns[inst].extend([{'active': False, 'note_count': 1, 'note_type': '1', 'repeats': 1} for _ in range(pattern_length - len(self.patterns[inst]))])
                elif len(self.patterns[inst]) > pattern_length:
                    self.patterns[inst] = self.patterns[inst][:pattern_length]
                # Upewniamy się, że każdy krok ma note_count
                for step in self.patterns[inst]:
                    if 'note_count' not in step:
                        step['note_count'] = 1
            else:
                if len(self.patterns[inst]) < pattern_length:
    ...
    def update_button_visual(self, button, instrument, step):
        if self.advanced_sequencer_mode:
            step_data = self.patterns[instrument][step]
            if step_data['active']:
                button.set_label(f"{step_data['note_count']} × {step_data['note_type']}\n×{step_data['repeats']}")
            else:
                button.set_label("")
        else:
            button.set_label("")
    ...
    def on_scroll(self, widget, event, instrument, step):
        ...
        # Lewy przycisk zmienia typ nuty
        ...
        # Środkowy przycisk zmienia liczbę nut (1-4)
        elif self.last_button_pressed == 2:
            current_count = step_data['note_count']
            new_count = max(1, min(4, current_count + (1 if scroll_direction == Gdk.ScrollDirection.UP else -1)))
            step_data['note_count'] = new_count
        # Prawy przycisk zmienia liczbę powtórzeń (1-16)
        elif self.last_button_pressed == 3:
            current_repeats = step_data['repeats']
            new_repeats = max(1, min(16, current_repeats + (1 if scroll_direction == Gdk.ScrollDirection.UP else -1)))
    ...
    def adjust_note_type(self, current_type, scroll_direction):
        note_types = ["1", "1/2", "1/4", "1/8", "1/16", "x2", "x4", "x8", "x16"]
    ...
    def on_sequencer_mode_switch(self, switch, gparam):
        ...
        if self.advanced_sequencer_mode:
            # Aktualizujemy istniejące advanced_patterns do nowej struktury
            for inst in self.instruments:
                for step in self.advanced_patterns[inst]:
                    if 'note_count' not in step:
                        step['note_count'] = 1  # Domyślna wartość dla istniejących danych
    ...
    def generate_custom_pattern(self, widget):
        ...
        # Dodajemy podstawowe reguły dla instrumentów
        for inst in self.instruments:
            for i in range(pattern_length):
                if inst == 'Stopa':
    ...
    def _apply_step(self, inst, step, genre, complex=False):
        ...
        if self.advanced_sequencer_mode:
            ...
            self.patterns[inst][step]['note_count'] = random.randint(1, 3) if complex else 1
            self.patterns[inst][step]['note_type'] = random.choice(rules[inst])
            self.patterns[inst][step]['repeats'] = random.randint(1, 4) if complex else random.randint(1, 2)
        else:
                ...
                for inst in self.instruments:
                    if self.advanced_sequencer_mode:
                        ...
                        if step_data['active'] and inst in self.samples:
                            ...
                            adjusted_duration = base_step_duration * playback_speed / step_data['note_count']
                            for _ in range(step_data['repeats']):
                                for _ in range(step_data['note_count']):
                                    modified_sound.play()
    ...
    def get_playback_speed(self, note_type):
        playback_speeds = {
            "1": 1.0, "1/2": 0.5, "1/4": 0.25, "1/8": 0.125, "1/16": 0.0625,
            "x2": 2.0, "x4": 4.0, "x8": 8.0, "x16": 16.0
        }
    ...
    def load_project(self, widget):
        ...
        if response == Gtk.ResponseType.OK:
            ...
            # Aktualizujemy advanced_patterns do nowej struktury
            old_advanced_patterns = project_data.get("advanced_patterns", {inst: [{'active': False, 'note_type': '1', 'repeats': 1} for _ in range(16)] for inst in self.instruments})
            self.advanced_patterns = {}
            for inst in self.instruments:
                self.advanced_patterns[inst] = []
                for step in old_advanced_patterns[inst]:
                    new_step = {
                        'active': step['active'],
                        'note_count': step.get('note_count', 1),  # Domyślnie 1, jeśli brak
                        'note_type': step['note_type'],
                        'repeats': step['repeats']
                    }
                    self.advanced_patterns[inst].append(new_step)
            ...
            if self.advanced_sequencer_mode:
                for i in range(pattern_length):
                    ...
                    if inst == 'Stopa':
                        ...
                        step_data['note_count'] = random.randint(1, 3)
                        step_data['note_type'] = random.choice(["1", "1/2", "1/4", "1/8"])
                        step_data['repeats'] = random.randint(1, 4)
                    elif inst == 'Werbel':
                        step_data['active'] = True if i % 4 == 2 else False
                        step_data['note_count'] = random.randint(1, 2)
                        step_data['note_type'] = random.choice(["1", "1/2"])
                        step_data['repeats'] = random.randint(1, 3)
                    elif inst == 'Talerz':
                        step_data['active'] = random.choice([True, False]) if not self.patterns['Stopa'][i]['active'] else False
                        step_data['note_count'] = random.randint(1, 3)
                        step_data['note_type'] = random.choice(["1/2", "1/4", "1/8"])
                        step_data['repeats'] = random.randint(1, 4)
                    elif inst == 'TomTom':
                        step_data['active'] = random.choice([True, False]) if i % 8 == 7 else False
                        step_data['note_count'] = random.randint(1, 2)
                        step_data['note_type'] = random.choice(["1", "1/4"])
                        step_data['repeats'] = random.randint(1, 3)
            else:
                for i in range(pattern_length):
                    if inst == 'Stopa':
