...
class DrumSamplerApp(Gtk.Window):
    ...
    def add_drummer_to_audio(self, widget):
        # Otwórz dialog wyboru pliku audio
        ...
        if response == Gtk.ResponseType.OK:
            ...
            # Wykrywanie MIDI struktury i generowanie ścieżki perkusyjnej
            percussion_track, original_audio, sr = self.generate_drum_track(audio_path)

            # Zapisywanie ścieżki perkusyjnej i scalonego pliku
    ...
    def save_generated_tracks(self, audio_path, percussion_track, original_audio, sr):
        """Zapisuje wygenerowane ścieżki: perkusyjną i scalony plik."""
        percussion_audio = self.synthesize_percussion_audio(percussion_track, sr)
    
        # Dopasowanie długości audio
        if len(percussion_audio) > len(original_audio):
            percussion_audio = percussion_audio[:len(original_audio)]
        elif len(percussion_audio) < len(original_audio):
            percussion_audio = np.pad(percussion_audio, (0, len(original_audio) - len(percussion_audio)))
    
        # Połączenie ścieżek
        combined_audio = original_audio + percussion_audio
    
        # Ścieżki wynikowe
        percussion_path = audio_path.replace(".mp3", "_drums.wav")
        combined_path = audio_path.replace(".mp3", "_combined.wav")
    
        print(f"Saving percussion track to: {percussion_path}")
        sf.write(percussion_path, percussion_audio, sr)
    
        print(f"Saving combined audio to: {combined_path}")
        sf.write(combined_path, combined_audio, sr)
    
        # Wyświetl potwierdzenie zapisu
        self.show_save_confirmation(percussion_path, combined_path)
