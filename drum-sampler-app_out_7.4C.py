...
class DrumSamplerApp(Gtk.Window):
    ...
    def detect_mood_and_modality(self, y, sr):
        """
        Zaawansowana analiza modalności i nastroju utworu
        """
        # Ekstrakcja chromatyki - analiza harmoniczna
        chroma = librosa.feature.chroma_cqt(y=y, sr=sr)
        
        # Analiza tonacji i trybu
        tonnetz = librosa.feature.tonnetz(y=y, sr=sr)
        
        # Spektralna centroida - jasność dźwięku
        spectral_centroid = librosa.feature.spectral_centroid(y=y, sr=sr)[0]
        
        # Alternatywna metoda oceny złożoności spektralnej
        spectral_bandwidth = librosa.feature.spectral_bandwidth(y=y, sr=sr)[0]
        
        # Analiza trybu Major/Minor
        major_minor_ratio = np.mean(chroma, axis=1)
        is_minor = np.argmax(major_minor_ratio) < len(major_minor_ratio) / 2
        
        # Klasyfikacja nastroju bazująca na parametrach
        mood = None
        if is_minor:
            if np.mean(spectral_centroid) < 1500:  # ciemniejsze brzmienie
                mood = 'melancholic'
            else:
                mood = 'dramatic'
        else:
            if np.mean(spectral_centroid) > 2000:  # jaśniejsze brzmienie
                mood = 'energetic'
            else:
                mood = 'calm'
        ...
        return {
            'is_minor': is_minor,
            'mood': mood,
            'spectral_complexity': np.mean(spectral_bandwidth)
        }

    def save_generated_tracks(self, audio_path, percussion_track, original_audio, sr):
        """
        Optimized track saving with progress tracking
        """
        # Compute mix more efficiently
        percussion_audio = self.synthesize_percussion_audio(percussion_track, sr, original_audio)
    ...
    def mix_tracks(self, original_audio, percussion_audio):
        """
        Dynamically mix the original audio with the generated percussion track.
        """
        original_rms = np.sqrt(np.mean(original_audio**2))
        percussion_rms = np.sqrt(np.mean(percussion_audio**2))
        mix_ratio = original_rms / (original_rms + percussion_rms)
    
        combined_audio = (
            original_audio * (1 - mix_ratio) + 
            percussion_audio * mix_ratio
        )
        ...
        for inst in self.instruments:
            ...
            for step in trigger_steps:
                try:
                    ...
                    # Rapid resampling with minimal quality loss
                    if len(sample_array) > step_duration:
                        sample_array = sample_array[:step_duration]
                    elif len(sample_array) < step_duration:
                        sample_array = np.pad(
                            sample_array, 
                            (0, max(0, step_duration - len(sample_array)))
                        )
                    
                    # Efficient placement in audio array
