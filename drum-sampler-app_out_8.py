...
class DrumSamplerApp(Gtk.Window):
    def __init__(self):
        ...
        self.advanced_sequencer_mode = False  # Domyślnie tryb prosty
        ...
        self.patterns = self.simple_patterns  # Domyślnie prosty tryb
    ...
    def update_buttons(self):
        ...
        # Upewniamy się, że self.patterns ma odpowiednią długość
            ...
            for i in range(pattern_length):
                try:
                    ...
                    if self.advanced_sequencer_mode:
                        ...
                        button.set_label("")  # W trybie prostym nie pokazujemy etykiet
                except IndexError:
                    print(f"IndexError: Button for {inst} at step {i} not found. Reinitializing buttons.")
                    self.reinitialize_buttons()
                    return  # Ponowne wywołanie po reinicjalizacji
    ...
    def reinitialize_buttons(self):
        """Reinicjalizacja przycisków w przypadku problemów z indeksami"""
        pattern_length = int(self.length_spinbutton.get_value())
        for inst in self.instruments:
            self.buttons[inst] = []
            for i in range(pattern_length):
        ...
        self.grid.show_all()
        self.update_buttons()  # Ponowne wywołanie po reinicjalizacji
    ...
    def on_sequencer_mode_switch(self, switch, gparam):
        ...
        self.update_buttons()
        print(f"Sequencer mode switched to: {'Advanced' if self.advanced_sequencer_mode else 'Simple'}")
    ...
    def create_pattern_controls(self):
        ...
        # Nowy ComboBox dla modyfikatora
        ...
        self.intensity_spin.set_numeric(True)  # Zapewnia, że tylko liczby są akceptowane
    ...
    def generate_custom_pattern(self, widget):
        genre = self.custom_genre_entry.get_text() or "Generic"  # Domyślny gatunek, jeśli pole jest puste
        ...
        pattern_length = int(self.length_spinbutton.get_value())
        mod = self.mod_combo.get_active_text() if hasattr(self, 'mod_combo') else "None"  # Nowy modyfikator
    
        # Inicjalizacja patternów w zależności od trybu
        if self.advanced_sequencer_mode:
            for inst in self.instruments:
        ...
        # Generowanie bazowego patternu w zależności od progresji
        ...
        # Modyfikacja patternu w zależności od "mod"
        if mod == "Simplify":
            for inst in self.instruments:
                for i in range(pattern_length):
                    if self.advanced_sequencer_mode:
            ...
            for inst in self.instruments:
                for i in range(pattern_length):
                    if random.random() < intensity * 0.2:  # Dodatkowe kroki z małą szansą
                        self._apply_step(inst, i, genre, complex=True)
    
        self.update_buttons()
        print(f"Generated pattern for {genre} with {progression}, {occurrences} occurrences, intensity {intensity}, mod {mod}")
    
    def _apply_step(self, inst, step, genre, complex=False):
        """Pomocnicza funkcja do aplikacji kroku w zależności od gatunku i trybu"""
        genre_rules = {
            ...
            if self.advanced_sequencer_mode:
                ...
                for i in range(pattern_length):
                    if i not in active_steps:
                        neighbors = [
                            self.patterns[instrument][(i - 1) % pattern_length]['active'],
                            self.patterns[instrument][(i + 1) % pattern_length]['active']
                        ]
                        if sum(neighbors) > 0 or random.random() < 0.3:
                            self.patterns[instrument][i]['active'] = random.choice([True, False]) if random.random() < 0.7 else False
                            self.patterns[instrument][i]['note_type'] = random.choice(["1", "1/2", "1/4"])
                            self.patterns[instrument][i]['repeats'] = random.randint(1, 2)
                if instrument == 'Stopa' and i % 4 == 0:
                    ...
                    self.patterns[instrument][i]['active'] = random.choice([True, False])
                ...
                for i in range(pattern_length):
                    if i not in active_steps:
                        neighbors = [
                            self.patterns[instrument][(i - 1) % pattern_length],
                            self.patterns[instrument][(i + 1) % pattern_length]
                        ]
                        if sum(neighbors) > 0 or random.random() < 0.3:
                            self.patterns[instrument][i] = random.choice([0, 1]) if random.random() < 0.7 else self.patterns[instrument][i]
                if instrument == 'Stopa' and i % 4 == 0:
                    self.patterns[instrument][i] = 1
                elif instrument == 'Werbel' and i % 8 == 4:
                    self.patterns[instrument][i] = 1
                elif instrument == 'Talerz' and random.random() < 0.2:
                    self.patterns[instrument][i] = random.choice([0, 1])
                elif instrument == 'TomTom' and i % 16 == 7:
                    self.patterns[instrument][i] = random.choice([0, 1])
        self.update_buttons()
    ...
    def reset_genre_fx(self, widget):
        for instrument in self.instruments:
            for effect in self.effects[instrument]:
                ...
                if effect in self.effect_sliders[instrument]:
                    self.effect_sliders[instrument][effect].set_value(0)
        print("Genre FX reset to default (0)")
